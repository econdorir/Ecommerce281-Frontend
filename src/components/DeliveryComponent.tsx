...
const DeliveryComponent = () => {
    ...
    const loadDeliveryDetails = async () => {
        try {
            const response = await fetch('http://localhost:5000/api/v1/entrega/detalles/1');
            if (!response.ok) {
    ...
    const confirmDeliveryByArtesano = async () => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/api/v1/entrega/confirmar/artesano/${deliveryDetails.id_entrega}`, {
                method: 'PUT',
                headers: {
    ...
    const confirmDeliveryByCliente = async () => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/api/v1/entrega/confirmar/cliente/${deliveryDetails.id_entrega}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
