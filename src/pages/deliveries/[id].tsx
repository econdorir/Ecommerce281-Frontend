    ...
    const fetchDeliveryDetails = async () => {
      ...
      try {
        const response = await fetch(
          `http://localhost:5000/api/v1/entrega/${id}`
        );
        if (!response.ok) {
  ...
  const confirmDelivery = async () => {
    ...
    try {
      ...
      if (
        ...
        const response = await fetch(
          `http://localhost:5000/api/v1/entrega/${deliveryDetails.id_entrega}`,
          {
            method: "PATCH",
            headers: {
      ...
      if (!deliveryDetails.delivery_confirm) {
        ...
        if (deliveryDetails.cliente_confirm) {
          ...
          const response2 = await fetch(
            `http://localhost:5000/api/v1/pedido/${deliveryDetails.pedido.id_pedido}`,
            {
              method: "PATCH",
              headers: {
                "Content-Type": "application/json",
    ...
    async (aniade: Aniade) => {
      ...
      try {
        ...
        if (aniade.delivery_confirm && !aniade.artesano_confirm) {
          window.confirm("¿Deseas cancelar la confirmación de la entrega?");
          const response = await fetch(
            `http://localhost:5000/api/v1/aniade/${aniade.id_aniade}`,
            {
              method: "PATCH",
              headers: {
                "Content-Type": "application/json",
